...
<div class="col-md-6">
            <div class="well bs-component">
              <form class="form-horizontal" action="http://technoflicker.com/smsgateway/check_api.php" method="post">
                ...
                <fieldset>
                  <legend>Send Bulk SMS To Parents</legend>
                  <div class="form-group">
                    
                  </div>

	                  <div class="form-group">
					      <label for="select" class="col-lg-4 control-label">Selects Multiple Parents [USE 'Ctrl' Key]</label>
					      <div class="col-lg-6">
					        <select multiple="multiple" class="form-control" name="mobile_no[]" id="select_class" data-rel="chosen" value="displaynone">
					        
					        <?php 

                  			$select_class = "SELECT * FROM parent";
							         $q= mysqli_query($con,$select_class);
							         while ($row=mysqli_fetch_array($q)) {
								 $parents_name = $row['name'];
								 $phone_no = $row['phone'];
						?>
					          <option value="<?php echo "$phone_no"; ?>"><?php echo "{$parents_name} ContactNo. {$phone_no}"; ?></option>
					          <?php }?>
					        </select></div>
					        <br>
